...
def getFeed(page = 1):
	...
	if page in feed_cache and (datetime.datetime.now() - feed_cache[page]["lastUpdated"]).total_seconds() < 60 * 30:
		return feed_cache[page]["feedString"]
	...
	if worldResponse.status_code != 200:
		...
		return feed_cache.get(page, {"feedString": ""})["feedString"]
